...
### Собственное имя рядом с туннелями

`-route` направляет фиксированное имя на локальный адрес вместо туннеля —
чтобы панель или статус-страница делили один TLS-листенер, а не требовали
реверс-прокси перед всем:
...
Имя должно быть в сертификате и вне туннельной зоны: маршрут внутри базового
домена отвергается, иначе он перекрыл бы туннель, а тот, кто это имя занял,
никогда бы не понял, почему его туннель недоступен.
